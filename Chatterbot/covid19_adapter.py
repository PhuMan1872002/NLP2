...
class Covid19Adapter(LogicAdapter):
    ...
    def process(self, input_statement, additional_response_selection_parameters):
        ...
        provinces = ['Ontario', 'Quebec', 'Manitoba', 'Newfoundland and Labrador', 'New Brunswick', 'British Columbia',
                     'Prince Edward Island', 'Saskatchewan', 'Yukon', 'Nova Scotia', 'Northwest Territories']

        today = date.today()
        days = 2
        response = requests.get(
            'https://api.covid19api.com/country/canada/status/confirmed?from={}&to={}'.format(today - timedelta(days),
                                                                                              today))
        ...
        if any(x in input_statement.text.split() for x in provinces):
            ...
            output_dict = [y for y in input_dict if
                           y['Province'] == list(set(input_statement.text.split()).intersection(provinces))[0]]
